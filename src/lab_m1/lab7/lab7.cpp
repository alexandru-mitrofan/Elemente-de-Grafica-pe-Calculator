...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    GLint loc_model_matrix = glGetUniformLocation(shader->program, "light_position");
    glUniform3fv(loc_model_matrix, 1, glm::value_ptr(lightPosition));


    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // TODO(student): Set eye position (camera position) uniform
    loc_model_matrix = glGetUniformLocation(shader->program, "eye_position");
    glUniform3fv(loc_model_matrix, 1, glm::value_ptr(eyePosition));

    // TODO(student): Set material property uniforms (shininess, kd, ks, object color)
    loc_model_matrix = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(loc_model_matrix, materialShininess);

    loc_model_matrix = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(loc_model_matrix, materialKd);

    loc_model_matrix = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(loc_model_matrix, materialKs);

    loc_model_matrix = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(loc_model_matrix, 1, glm::value_ptr(color));
    // Bind model matrix
